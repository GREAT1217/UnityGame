...
namespace GameMain.Editor.DataTable
{
    public sealed class DataTableGenerator
    {
        ...
        private const string CollectionFileName = "Assets/GameMain/Configs/Editor/DataTableCollection.json";
        ...
        /// 生成数据表Text文件
        /// </summary>
        [MenuItem("Generator/Generate DataTable Text File", false, 41)]
        private static void GenerateDataTableTextFile()
        {
            ExcelHelper.BatchExcelToText(ExcelPath, AssetUtility.DataTablePath, CollectionFileName);
        ...
        /// 生成数据表数据文件和代码文件
        /// </summary>
        [MenuItem("Generator/Generate DataTable Data And Code File", false, 42)]
        private static void GenerateDataTableDataAndCodeFile()
        {
            ...
            if (!Directory.Exists(AssetUtility.DataTablePath))
            {
                Directory.CreateDirectory(AssetUtility.DataTablePath);
            }
            ...
            List<string> dataTableNames = JsonMapper.ToObject<List<string>>(collection);
            for (int i = 0; i < dataTableNames.Count; i++)
            {
                ...
                try
                {
                    DataTableProcessor dataTableProcessor = new DataTableProcessor(Utility.Path.GetRegularPath(AssetUtility.GetDataTableAsset(dataTableName, false)), Encoding.GetEncoding("GB2312"), 1, 2, null, 3, 4, 1);
        ...
        /// 生成数据表数据文件
        /// </summary>
        [MenuItem("Generator/Generate DataTable Data File", false, 43)]
